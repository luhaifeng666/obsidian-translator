...
async function handleMicrosoftTranslate(
	q: string,
	config: {
		secretKey: string;
		from?: string;
	...
	axios({
		...
		.then(({ data }) => {
			const res = (data || []).reduce(
				(str: string, item: MicrosoftTranslations) => {
...
async function handleBaiduTranslate(
	q: string,
	config: {
		appId: string;
		secretKey: string;
	...
	getRequest(
		`http://api.fanyi.baidu.com/api/trans/vip/translate?q=${q}&from=${
			from || "en"
		}&to=${to || "zh"}&appid=${appId}&salt=${salt}&sign=${sign}`,
		cb
	);
}
...
function getLanguageOptions(languages: LanguageOptions): LanguageOptions {
	return Object.keys(languages).reduce(
		(obj: LanguageOptions, key: string) => ({
			...obj,
...
function validator(
	items: Array<{ value: string; message: string }>,
	cb: () => void
) {
